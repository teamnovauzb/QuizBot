...
import { PlusIcon, XIcon, ClockIcon, TrashIcon } from '../../components/Icons'
import { fetchAssignments, createAssignment, deleteAssignment, type AssignmentRow } from '../../lib/api2'
          ...
          <Card key={a.id} className="p-4">
            <div className="flex items-baseline justify-between gap-2 mb-1">
              ...
              <button onClick={async () => {
                ...
                await deleteAssignment(a.id); haptic('heavy'); reload()
      ...
      <AnimatePresence>
        {creating && (
          <CreateAssignmentSheet
            ...
            onCreate={async (data) => {
              const r = await createAssignment(data)
              if (r.ok) reload()
              setCreating(false)
            }}
          />
...
function CreateAssignmentSheet({ onClose, onCreate, groupId, availableQuestions }: {
  onClose: () => void
  groupId?: string
  availableQuestions: { id: string; question: string; category: string }[]
  onCreate: (data: { title: string; description: string | null; group_id: string | null; question_ids: string[]; time_per_q: number; deadline: string | null }) => void
}) {
  const { t } = useTranslation()
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div className="px-5 py-3 flex items-center justify-between">
          <h3 className="font-display text-2xl">{t('admin.newAssignment')}</h3>
        ...
        <div className="flex-1 overflow-y-auto px-5 space-y-3">
          ...
          <div>
            <div className="text-[10px] uppercase font-mono tracking-[0.18em] text-[var(--ink-soft)] mb-1.5">{t('admin.pickQuestions', { n: picked.size })}</div>
            <ul className="space-y-1.5 max-h-[40vh] overflow-y-auto">
              {availableQuestions.slice(0, 60).map(q => (
        ...
        <div className="px-5 pt-3">
          <button
            onClick={() => onCreate({
              title, description: description || null, group_id: groupId ?? null,
              question_ids: Array.from(picked), time_per_q: time,
